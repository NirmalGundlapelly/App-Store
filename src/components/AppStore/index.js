...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, enteredText: ''}
  ...
  render() {
    const {activeTabId, enteredText} = this.state
    const filteredApps = appsList.filter(
      eachItem =>
        eachItem.category === activeTabId &&
        eachItem.appName.toLowerCase().includes(enteredText.toLowerCase()),
    )
    return (
          ...
          <div className="tabContainer">
            ...
            <ul className="appItemsContainer">
              {filteredApps.map(eachApp => (
                <AppItem appListItems={eachApp} key={eachApp.appId} />
              ))}
            </ul>
          </div>
        </div>
